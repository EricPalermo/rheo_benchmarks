...
void Set::command(int narg, char **arg)
{
  ...
  while (iarg < narg) {
    ...
    if (strcmp(arg[iarg],"type") == 0) {
      ...
      if (!atom->rho_flag)
        error->all(FLERR,"Cannot set rho for this atom style");
      set(RHO);
      iarg += 2;
      ...
      if (!atom->temp_flag)
        error->all(FLERR,"Cannot set temp for this atom style");
      set(TEMP);
      iarg += 2;
      ...
      if (!atom->rho_flag)
        error->all(FLERR,"Cannot set phase for this atom style");
      set(PHASE);
      iarg += 2;      
...
void Set::set(int keyword)
{
  ...
  for (int i = 0; i < nlocal; i++) {
    ...
    else if (keyword == RHO) atom->rho[i] = dvalue;
    else if (keyword == TEMP) atom->temp[i] = dvalue;
    else if (keyword == PHASE) atom->phase[i] = ivalue;
