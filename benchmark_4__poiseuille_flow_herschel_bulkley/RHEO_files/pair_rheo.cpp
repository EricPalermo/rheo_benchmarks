  ...
  for (ii = 0; ii < inum; ii++) {
    ...
    for (jj = 0; jj < jnum; jj++) {
      ...
      if (rsq < cutsq[itype][jtype]) {
        ...
        if (thermal_flag) {
          dT = 0.0;
          if (laplacian_order == 2 && error_flag == 0) {
            for (a = 0; a < dim; a++)
              dT += d2Wij[a];
            dT *= voli*volj*0.5*(kappai+kappaj)*(Tj-Ti);
          } else if (laplacian_order == 1) {
            for (a = 0; a < dim; a++) {
            ...
            dT = voli*volj*5.0/6.0*wp*ir*(kappai+kappaj)*(Ti-Tj);
          }
          heat[i] += dT;
        }
        ...
        if (pair_force_flag) {
          ...
          if (laplacian_order == 2 && error_flag == 0) {
            for (a = 0; a < dim; a ++) {
              ...
              for(b = 0; b < dim; b++)
                fv[a] += d2Wij[b];
              fv[a] *= voli*volj*0.5*(etai+etaj)*(vj[a]-vi[a]);
            }
          } else if (laplacian_order == 1) {
            for (a = 0; a < dim; a ++) {
              fv[a] = 0.0;
              for (b = 0; b < dim; b++) {
                //fv[a] += etaj*dWij[b]*(gradv[j][a*dim+b] + gradv[j][b*dim+a]);
                //fv[a] -= etai*dWij[b]*(gradv[i][a*dim+b] + gradv[i][b*dim+a]);
            ...
            dv[1] = vi[1] - vj[1];
            dv[2] = vi[2] - vj[2];
            vijeij = (dx[0]*dv[0] + dx[1]*dv[1] + dx[2]*dv[2])*ir;

            fv[0] = voli*volj*5.0/6.0*wp*ir*(etai+etaj)*(dv[0] + vijeij*dx[0]*ir);
            fv[1] = voli*volj*5.0/6.0*wp*ir*(etai+etaj)*(dv[1] + vijeij*dx[1]*ir);
            fv[2] = voli*volj*5.0/6.0*wp*ir*(etai+etaj)*(dv[2] + vijeij*dx[2]*ir);
          }
        } else {
          for (a = 0; a < dim; a ++) {
            fv[a] = 0;
        ...
        if (newton_pair || j < nlocal) {
          ...
          if (thermal_flag) {
            if (laplacian_order == 2 && error_flag == 0) {
              ...
              for(a = 0; a < dim; a++)
                dT += d2Wji[a];
              dT *= -voli*volj*0.5*(kappai+kappaj)*(Ti-Tj);
            } else if (laplacian_order == 1) {
              dT = 0.0;
              for(a = 0; a < dim; a++){
          ...
          // Compute dimension and laplacian dependent dissipative parameters:
          if (laplacian_order == 2 && error_flag == 0) {
            for (a = 0; a < dim; a ++) {
              fv[a] = 0.0;
              for(b = 0; b < dim; b++)
                fv[a] += d2Wji[b];
              fv[a] *= -voli*volj*0.5*(etai+etaj)*(vi[a]-vj[a]);
            }
          } else if (laplacian_order == 1) {
            for (a = 0; a < dim; a ++) {
              fv[a] = 0.0;
